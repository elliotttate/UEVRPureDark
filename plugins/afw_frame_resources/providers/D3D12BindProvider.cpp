...
// clear). That keeps the pointer alive between the CreateView detour that records it and the
// OMSetRenderTargets detour that promotes it, closing the use-after-free window.
...
D3D12ViewInfo describe(ID3D12Resource* resource) {
    ...
    v.flags = d.Flags;
    v.uevr_owned = resource_debug_name_contains(resource, L"UEVR");
    return v;
}
...
void consider_created_velocity_resource(ID3D12Resource* resource, const char* source) {
    ...
    const D3D12ViewInfo info = describe(resource);
